...
export const load: LayoutServerLoad = async ({ locals }) => {
	...
	let home_required = false;
	let current_user_image: string | null = null;
	let is_verified = false;

	if (is_authenticated && locals.user?.id) {
		const result = await db.execute({
			sql: 'SELECT username, image, verified, email, emailVerified, home_lat FROM user WHERE id = ? LIMIT 1',
			args: [locals.user.id]
		});
		...
		// No coordinates means no distance, so pigeon post stays locked until the
		// user grants location. The rest of the app is unaffected.
		home_required = row?.home_lat === null || row?.home_lat === undefined;
