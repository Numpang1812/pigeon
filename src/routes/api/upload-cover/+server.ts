/**
 * API Route: Upload Cover Photo
 *
 * Accepts a compressed image file, uploads to Cloudinary,
 * and updates the user's cover photo URL in the database.
 *
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const new_cover_url = await upload_cover_photo(user_id, file_buffer);

		// 5. Update user's cover in database
		...
		throw error(500, `Failed to upload cover: ${message}`);
	}
};
