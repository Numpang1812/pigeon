<script lang="ts">
	...
	async function handle_submit(e: SubmitEvent) {
		...
		try {
			...
			success = true;
			// Navigate to login or directly sign in; replace with goto("/login") if needed
		} catch (err) {
...
<svelte:head>
	<title>Sign up · Pigeon</title>
	<meta name="description" content="Join Pigeon, the minimalist microblogging platform." />
...
<main class="signup-root">
	...
	<section class="brand-panel" aria-hidden="true">
		<div class="brand-content">
			...
			<h1 class="brand-headline">Join Pigeon today.</h1>
			<p class="brand-sub">The conversation starts here.</p>
		</div>
			...
			{:else}
				<form id="signup-form" onsubmit={handle_submit} novalidate>
					...
					<button
						...
						type="submit"
						class="btn-primary"
						disabled={loading || !name || !username || !email || !password || !confirm_password}
					>
						{#if loading}
				...
				<p class="login-prompt">
					...
					<a href={resolve('/')} class="login-link">Sign in</a>
				</p>
			{/if}
		</div>
	</section>
